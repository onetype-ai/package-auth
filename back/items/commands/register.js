...
commands.Item({
    ...
    {
        const manifest = this.http.state.packages['onetype/auth'];
        ...
        if(manifest.limits['users:total'] !== null && await platform.users.Find().filter('deleted_at', null, 'NULL').count() >= manifest.limits['users:total'])
        {
    ...
    {
        if(!onetype.validate.email(properties.email))
        {
            return resolve(null, 'Please provide a valid email address.', 400);
        }

        const validation = onetype.validate.password(properties.password);
        ...
        const result = await pipelines.run('auth:register', {
